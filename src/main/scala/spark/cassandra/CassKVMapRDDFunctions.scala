...
class CassKVMapRDDFunctions[K <: Map[String, Any]: ClassManifest, V <: Map[String, Any]: ClassManifest](self: RDD[(K, V)])
  extends Logging
  with HadoopMapReduceUtil
  with Serializable {

  def debugCassKVMapRDDFunctionsKeyClassValueClass() = {
    println("[(K, V)] = [(" + getKeyClass + ", " + getValueClass + ")]")
  }

  def storeToCassandra(host: String, cql: String): Unit = {
    val outputFormatClass = classOf[MapTypeCql3OutputFormat]
